...
describe("extractSequenceFromFilename()", () => {
  test.each([["001.sql", 1], ["002-test.sql", 2], ["0003.sql", 3], ["1000.sql", 1000]])(
    "should return correct sequence for %p",
    (input, expected) => expect(migration.extractSequenceFromFilename(input)).toEqual(expected)
  );
});
...
describe("getPendingMigrations()", () => {
  test.each([
    [
      ...
      [{ seq: 2, filename: "002.sql" }, { seq: 1, filename: "001.sql" }],
      [{ seq: 2, filename: "002.sql" }]
    ],
    [
      [{ seq: 1, filename: "001.sql" }],
      [{ seq: 2, filename: "002.sql" }, { seq: 1, filename: "001.sql" }, { seq: 3, filename: "003.sql" }],
      [{ seq: 2, filename: "002.sql" }, { seq: 3, filename: "003.sql" }]
    ]
...
describe("generateNextMigrationFilename()", () => {
  test.each([[[], [], "001.sql"], [["001.sql"], [], "002.sql"], [["001.sql"], ["test"], "002-test.sql"]])(
    "should return correct filename for %p, %p",
    (migrations, name, expected) => expect(migration.generateNextMigrationFilename(migrations, name)).toEqual(expected)
  );
});

describe("validateStateConsistency()", () => {
  test("should throw when two files have the same sequence", () => {
    const local = [{ seq: 1, filename: "001.sql" }, { seq: 2, filename: "002.sql" }, { seq: 2, filename: "002-b.sql" }];
  ...
  test("should throw when local migration is missing", () => {
    const applied = [{ seq: 1, filename: "001.sql" }, { seq: 2, filename: "002.sql" }];
  ...
  test("should throw when hash is different", () => {
    const applied = [{ seq: 1, filename: "001.sql", hash: "a" }, { seq: 2, filename: "002.sql", hash: "b" }];
    const local = [{ seq: 1, filename: "001.sql", hash: "a" }, { seq: 2, filename: "002.sql", hash: "c" }];
